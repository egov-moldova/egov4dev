...
1. The user uploads the blob ([click here](/api-references/#blobs)).
2. The user publishes the document with reference to the blob ([click here](/api-references/#documents)).
3. The user will create a folder ([click here](/api-references/#documents)) - for the case when the document has no reference to the blob.
...
MDocs accepts client certificates generated by Certification Authority of the Information Technology and Cyber Security Service (<https://stisc.gov.md>), by requesting a certificate for authentication for systems.
